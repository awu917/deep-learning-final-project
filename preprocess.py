...
def fill_in_points(arr,err_arr):
	...
	* err_arr is the array of flux errors
	...
	if len(ind)==0:
		...
		err_arr = [0 for i in range(length)]
	else:
...
def parser_spline(filename):
	...
	* obs is a sequence of arrays each element containing [time since first observation,fluxes in each colourband,flux errors in each colourband]
	...
	obs = [[t[i],g_spline(t[i]).tolist(),r_spline(t[i]).tolist(),i_spline(t[i]).tolist(),z_spline(t[i]).tolist(),g_arr[2][index_min(abs(g_arr[0]-t[i]))],r_arr[2][index_min(abs(r_arr[0]-t[i]))],i_arr[2][index_min(abs(i_arr[0]-t[i]))],z_arr[2][index_min(abs(z_arr[0]-t[i]))]] for i in range(len(t))]
	return survey, snid, sn_type, sim_type, sim_z, ra, decl, mwebv, hostid, hostz, spec, obs
...
def parser_augment(filename):
	...
	* obs is a sequence of arrays each element containing [time since first observation,fluxes in each colourband,flux errors in each colourband]
	...
	obs = [[t[i],g_temp_arr[i],r_temp_arr[i],i_temp_arr[i],z_temp_arr[i],g_err_temp_arr[i],r_err_temp_arr[i],i_err_temp_arr[i],z_err_temp_arr[i]] for i in range(len(t))]
	return survey, snid, sn_type, sim_type, sim_z, ra, decl, mwebv, hostid, hostz, spec, obs
...
if __name__ == '__main__':
	...
	print('Num train: ' + str(nb_sn))
	print('SN types: ' + str(sn_types))